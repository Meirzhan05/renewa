...
// downgraded to low. This is what lets the reasoner assert fields more freely (needed to turn
// a cycle-less receipt into "ask") without importing hallucination risk. Amounts are the
// exception the reasoner already guards: never inferred, and here re-checked like any field.
...
// Fields the verifier grounds (only those the assessment actually asserts are checked).
...
/**
 * Apply a verdict to an assessment. Pure. Strips any asserted checkable field the verifier
 * did not ground, downgrades existence to "low" when unsupported, and recomputes
 * completeness/missing_fields so routing sees the grounded picture.
 */
...
/**
 * Run the verification call. On any error or unparseable verdict the assessment passes
 * through UNCHANGED — a verifier outage degrades precision gracefully rather than dropping a
 * real subscription (consistent with the classifier's safe-degradation stance).
 */
export async function verifyAssessment(
  ...
  // Nothing to ground if the reasoner asserted no checkable fields and existence is low.
