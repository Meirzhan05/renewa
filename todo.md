...
- [x] Animate tab changes with a direction-neutral crossfade and a moving active-tab accent, while respecting Reduce Motion.
...
- [ ] Deploy and production-verify coordinated completion for paginated Inbox scans, so a completed early worker page cannot end the shared scan while later pages still run.
- [ ] Replace the local Inbox agent worker with a deployed managed workflow runtime that can fairly process concurrent App Store users, recover interrupted work, and keep credentials out of task payloads. The feature-gated Trigger.dev workflow, durable ledger, cancellation UI, and local tests are implemented; the development runner is registered and its REST-endpoint configuration now supports both project-root and legacy `/rest/v1` URLs. Staged end-to-end and 100-user load verification remain.
