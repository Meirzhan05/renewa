...
- [ ] Replace the local Inbox agent worker with a deployed managed workflow runtime that can fairly process concurrent App Store users, recover interrupted work, and keep credentials out of task payloads. The development runtime now processes one page at a time per user through durable child-task checkpoints, so capacity waits do not falsely time out after five minutes. Staged end-to-end and 100-user load verification remain.
