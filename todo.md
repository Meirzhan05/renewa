...
- [x] Remove inactive subscriptions from the Home feed while retaining their data for history and Insights.
...
- [ ] Replace the local Inbox agent worker with a deployed managed workflow runtime that can fairly process concurrent App Store users, recover interrupted work, and keep credentials out of task payloads. The durable dispatcher and status/Stop UI are implemented; development migration, Trigger deployment, and staged multi-user verification remain.
